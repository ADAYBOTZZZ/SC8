...
//PAIRING CODE BY FluXz Official

CFonts.say("SC PAIRING BY FluXz Official", {
   font: 'chrome',
  align: 'left',
  gradient: ['red', 'magenta'],
});
CFonts.say(
  "Sc Bot By FluXz\nBase: Ori FluXzOffcial\nRecodeBy:FluXzoffcial\ncampuranv:8\nRuning:Panel\n\n\nSc Ini Di Recode FluXz Official Kalo Kethauan Jual Murah Sc inI gua Banned Nyawa Lu Jarak Jauh\n\n\n",
  {
    colors: ["system"],
    font: "console",
    align: "center",
...
if(usePairingCode && !Biiofc.authState.creds.registered) {
		const phoneNumber = await question(color('\n\n\nSilahkan masukin nomor  Awali dengan 62:\n', 'magenta'));
		const code = await Biiofc.requestPairingCode(phoneNumber.trim())
		console.log(color(`⚠︎ Kode Pairing Bot Whatsapp kamu dari bang FluXz:`,"black"), color(`${code}`, "white"))
...
//await sleep(400) 
console.log(`${color(`[`,`white`)+color(`3`,`red`)+color(`]`,`white`)}`,`Base Sc: FluXz`)
//await sleep(400)  
console.log(`${color(`[`,`white`)+color(`4`,`red`)+color(`]`,`white`)}`,"date 5") 
//await sleep(400)  
console.log(color(`─[`,`magenta`),`「`,  color(`FluXz`,`red`), `」`,  color(`]─`,`magenta`))
//await sleep(400)  
start(`1`,`Connecting...`)
